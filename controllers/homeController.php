...
$visibilitycal = $_GET["visibilitycal"]??'0';
$title = $_POST["title"]??'';
...
if(isset($_GET['id']) AND !empty($_GET['id'])) {
    ...
    if(isset($_GET['funct']) AND !empty($_GET['funct'])) {
        ...
        elseif ($funct=="edit")
        {
        $editevents=Editevents($id);
        }
        elseif ($funct=="save")
        {
        Saveevents($id, $title, $description, $visibility);
        }
    }
}
else
...
if(isset($_GET['idcaldel']) AND !empty($_GET['idcaldel'])) {
    $idcal = htmlspecialchars($_GET['idcaldel']);
    if(isset($_GET['funct']) AND !empty($_GET['funct'])) {
        $funct = htmlspecialchars($_GET['funct']);
        if ($funct=="delagenda"){ 
        Delagenda($idcal);
...
if(isset($_GET['monthnew']) AND !empty($_GET['monthnew'])) {
    $monthnew = htmlspecialchars($_GET['monthnew']);
}
else
{
    $monthnewNum = date_parse(htmlspecialchars($_GET['month']));
    $monthnew = $monthnewNum['month'];
}
...
//$divtwig2 ="<div></div>";
//for ($c=1; $c < $startday; $c++) {
//    $divtwig2 = "{$divtwig2}<div></div>" ;
// }
...
//$usercreator = usercreator($user['ID']);
//echo $usercreator[0]['name'];
...
echo $template->render([
    'months'=>$months,
    'user'=>$user['name'],
 //   'usercreators'=>$usercreator[0]['name'],
    'month'=>$month,
